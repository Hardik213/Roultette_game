...
void Roulette_game::bet_type(){
    ...
    for (size_t i = 0; i < N; i++){
        ...
        case 4:
            break;
        default:
            cout<<"This is not a bet type option. Choose an appropriate option."<<endl<<endl;
            betway.erase(betway.begin(),betway.end());
            goto label1;
        }
    }
    cout<<"\n";
...
void Roulette_game::New_players(){
    ...
    if(N<5){
        ...
        if(Add_player=='y'||Add_player=='Y'){
            ...
            for (size_t i = N-New+1; i < N; i++){
                int x;
                cout<<"Player "<<i<<"("<<names.at(i)<<"), Enter the Amount of chips you want: ";
                cin>>x;
